...
Note that `read.table()` automatically skips lines that start with the comment character `#`, so we don't have to manually pass a parameter to tell the function to skip the header. Now some first-pass data exploration:
...
So to grab the last 5 years of data, we only want values of `co2$decimal_date` greater than 2021. We can use a conditional statement `>` to signal values *greater than* 2021:

```{r}
# Showing 2 ways to subset
summary(co2$decimal_date > 2021) # vector of TRUES and FALSES
summary(which(co2$decimal_date > 2021)) # vector of indices that meet condition (see ?which for bells and whistles)

# Either method can be used to subset the data
co2_2021to2026 = co2[co2$decimal_date > 2021,]
co2_2021to2026 = co2[which(co2$decimal_date > 2021),]
summary(co2_2021to2026)

plot(seasonal_cycle ~ decimal_date, type="l", data=co2_2021to2026) 
```
...
So it looks like peak CO2 levels occur in May, and the lowest CO2 levels occur in October. There seems to be an average range of about 6 ppm CO2 in a given year. This is related to global autotrophic respiration rates. 
