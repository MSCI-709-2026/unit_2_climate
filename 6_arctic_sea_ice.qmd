...
- Raw data from <https://noaadata.apps.nsidc.org/NOAA/G02135/north/daily/data/N_seaice_extent_daily_v4.0.csv>
...
```{r}
# arctic_ice = read.csv("data/N_seaice_extent_daily_v4.0.csv", skip=2, header = FALSE, col.names = c("Year", "Month", "Day", "Extent", "Missing", "Source_Data"))
...
arctic_ice = read.csv(url, skip=2, col.names = c("Year", "Month", "Day", "Extent", "Missing", "Source_Data"))
tail(arctic_ice)
```
...
First, we need to install the `lubridate` package from the CRAN library. You can do this in your R Console with the function `install.packages("lubridate")`. Once installed, you need to load the package functions into your environment with the `library()` function. Now that the `lubridate` package is loaded up, we can use the function `make_date()` to transform your year, month and day into a `date` type variable. Don't forget you can use the command `?make_date` to find out more about the function.

```{r}
# install.packages("lubridate") # do this once
library("lubridate")  # make_date() # do this every R session
...
So from 1978 to `r arctic_ice_averages$Year[dim(arctic_ice_averages)[1]]`, we have lost `r ice_loss_million_km2` million km^2 of ice extent in the Arctic. That's about the size of Alaska! 
